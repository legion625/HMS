...
//	private static Logger log = LoggerFactory.getLogger(AbstractHmsKernelInitTest.class);
	...
	public static void setupBeforeClass() throws Exception {
		HmsKernelInitLogTest.setupBeforeClass();
		assertNotNull("log null", log);
		log.debug("AbstractLegionInitTest setupBeforeClass...");
		// 啟始系統設定
		if (!initFlag) {
			synchronized (AbstractHmsKernelInitTest.class) {
				if (!initFlag) {
					init();
					initFlag = true;
				}
	...
	public static void init() {
		...
		/* datasource */
		String datasourceFile = "srcTest\\hms_kernel\\conf\\datasource.xml";
		/* service module */
		String serviceModuleFile = "srcTest\\hms_kernel\\conf\\ServiceModule.xml";
